...
torch::Tensor positional_encoding_1d(int d_model, int length, float base) {
    ...
    if ((d_model % 2) != 0) {
        throw (std::stringstream() << "Cannot use sin/cos positional encoding with odd dim (got dim=" << d_model
                                   << ")").str();
    }
...
std::vector<torch::Tensor> apply_rotary_position2d_embeddings(torch::Tensor sinusoidal,
    ...
    reshaped_tensors = apply_rotary_position_embeddings(std::move(sinusoidal), reshaped_tensors);
    std::vector<torch::Tensor> reverse_tensors;
    for (auto t: reshaped_tensors) {
        reverse_tensors.push_back(t.reshape({B, H, W, D}).permute({0, 3, 1, 2}).contiguous());
    }
    return reverse_tensors;
}

torch::Tensor Rotary2D::forward(torch::Tensor x) {
