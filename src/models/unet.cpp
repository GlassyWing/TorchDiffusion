...
AttentionBlockImpl::AttentionBlockImpl(int channels, int num_heads, int num_head_channels,
                                       bool use_checkpoint, bool use_new_attention_order): rot_pos(Rotary2D(channels)) {
    ...
    if (num_head_channels == -1) {
        ...
        assert(channels % num_head_channels == 0 &&
                       (std::stringstream() << "q,k,v channels " << channels << " is not divisible by num_head_channels " << num_head_channels)
                       .str().c_str());
...
void AttentionBlockImpl::zero_init_weights() {
    ...
    for (auto p: proj_out->parameters()) {
        p.zero_();
    }
}
